...
def create_network(
    ...
    network_dim: Optional[int],
    network_alpha: Optional[float],
    vae: nn.Module, #is not accessed : vae
    text_encoders: List[nn.Module],
    unet: nn.Module,
    neuron_dropout: Optional[float] = None,
    **kwargs,
...
class LoRANetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        target_replace_modules: List[str],
        prefix: str,
        text_encoders: Union[List[CLIPTextModel], CLIPTextModel], #is not accessed : text_encoders
        unet: nn.Module,
        ...
        def create_modules(
            is_unet: bool, #is not accessed : is_unet
            pfx: str,
    ...
    def apply_to(
        self,
        text_encoders: Optional[nn.Module], #is not accessed : text_encoders
        unet: Optional[nn.Module], #is not accessed : unet
    ...
    def merge_to(self, text_encoders, unet, weights_sd, dtype=None, device=None, non_blocking=False): #is not accessed : text_encoders, unet
    ...
    def prepare_optimizer_params(self, unet_lr: float = 1e-4, **kwargs): #is not accessed : kwargs
        # --- AÑADE ESTAS LÍNEAS PARA IMPRIMIR KWARGS ---
        print("--- KWARGS PARAMETERS BEGIN ---")
        if not kwargs: # Verifica si el diccionario está vacío
            print("No additional keyword arguments were passed.")
        else:
            # Iterar sobre los pares clave-valor del diccionario kwargs
            # No usamos enumerate aquí porque el "índice" de los kwargs no es tan relevante
            # como la clave y el valor.
            for key, value in kwargs.items():
                 print(f"Key: {key} : Value: {value}") # Usando f-strings para mejor formato
        print("--- KWARGS PARAMETERS END ---")
    ...
    def prepare_grad_etc(self, unet): #is not accessed : unet
        self.requires_grad_(True)

    def on_epoch_start(self, unet): #is not accessed : unet
        self.train()
